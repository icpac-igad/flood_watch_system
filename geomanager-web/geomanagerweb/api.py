...
class MultimodalForecastGeoJSONView(View):
    ...
    def get(self, request):
        ...
        with connection.cursor() as cursor:
            try:
                # Build the query to get GeoJSON with hybas_id added to each feature
                # This uses a LATERAL join to look up the basin for each point
                if date_str:
                    try:
                        date_obj = datetime.strptime(date_str, '%Y-%m-%d').date()
                        date_filter = "data_date = %s"
                        params = [date_obj]
                    except ValueError:
                        response = JsonResponse({
                            'error': 'Invalid date format. Use YYYY-MM-DD'
                    ...
                    date_filter = "data_date = (SELECT MAX(data_date) FROM home_multimodal_forecast_geojson)"
                    params = []

                # Query that adds hybas_id to each feature using ST_Within
                cursor.execute(f"""
                    WITH source_data AS (
                        SELECT geojson_data
                    ...
                    features_with_basin AS (
                        SELECT
                            jsonb_build_object(
                                'type', 'Feature',
                                'geometry', f.value->'geometry',
                                'properties', (f.value->'properties') || jsonb_build_object(
                                    'hybas_id', (
                                        SELECT h.hybas_id::bigint
                                        FROM gha.hydrobasins_lev06 h
                                        WHERE ST_Within(
                                            ST_SetSRID(ST_GeomFromGeoJSON((f.value->'geometry')::text), 4326),
                                            h.geom
                                        )
                                        LIMIT 1
                                    )
                        ...
                        FROM source_data,
                             jsonb_array_elements(geojson_data->'features') as f(value)
                    )
                    SELECT jsonb_build_object(
                        'type', 'FeatureCollection',
                        'features', COALESCE(jsonb_agg(feature), '[]'::jsonb)
                    )
                    FROM features_with_basin
                """, params)
                ...
                # The result is already a dict from PostgreSQL JSONB
                geojson = result[0]
        ...
        with connection.cursor() as cursor:
            try:
                ...
                if date_str:
                    try:
                        date_obj = datetime.strptime(date_str, '%Y-%m-%d').date()
                        date_filter = f"data_date = '{date_obj}'"
                    except ValueError:
                        response = JsonResponse({
                            'error': 'Invalid date format. Use YYYY-MM-DD'
                        }, status=400)
