...
def ingest_multimodal_geojson(data_date, date_string, geojson_data, feature_count, matched_count):
    """
    Ingest multimodal forecast GeoJSON into database
    ...
    try:
        with get_db_connection() as conn:
            ...
            if existing:
                # Update existing
                logger.info(f"Updating existing multimodal record for {data_date}")
                cursor.execute("""
                ...
                # Insert new
                logger.info(f"Creating new multimodal record for {data_date}")
                cursor.execute("""
