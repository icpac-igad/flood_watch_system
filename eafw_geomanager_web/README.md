...
The project relies on github authentication key configured on your computer. To ensure security and prevent leaking ssh keys into docker container layers, the ssh key is never copied into the container. Instead, ssh-key-agent is used to authenticate in-session. To ensure privately hosted repositories are properly installed, kindly run bellow commands before docker build.  

Ensure that your SSH agent is running and your private key is added to it on the host system before running the build
...
This method ensures that the private SSH key remains on the host and is only used during the build process, enhancing security by preventing the key from being embedded in the Docker image or cached layers. Please note that private key notation `id_ed25519` must be changed accordingly depending on how GitHub authentication ssh-key was generated.  
