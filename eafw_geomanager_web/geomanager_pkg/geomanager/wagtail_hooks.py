...
def register_geomanager_datasets(request):
    ...
    boundary_datasets = create_boundary_dataset(boundary_tiles_url)

    if isinstance(boundary_datasets, list):
        datasets.extend(boundary_datasets)
    else:
        datasets.append(boundary_datasets)
