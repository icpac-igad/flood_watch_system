...
def get_mapviewer_config(request):
    ...
    filter_types_data = []
    for ft in gm_settings.filter_types:
        data = ft.block.get_api_representation(ft.value)
        filter_types_data.append(data)
    if filter_types_data:
        response.update({"filterTypes": filter_types_data})
