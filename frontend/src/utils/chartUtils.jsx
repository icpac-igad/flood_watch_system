...
// Component to render Ensemble forecast charts (multiple models)
export const EnsembleForecastChart = ({ forecasts, height = 400 }) => {
  const chartRef = useRef(null);

  if (!forecasts || forecasts.length === 0) {
    return <div className="chart-no-data" style={{ padding: '20px', textAlign: 'center' }}>No forecast data available.</div>;
  }

  // Process forecast data: convert date strings to Date objects and parse numeric values
  const chartData = React.useMemo(() => {
    return forecasts.map(record => {
      ...
      // Parse all available forecast models
      const models = ['Floodproof', 'GeoSFM', 'daily_avg', 'daily_max', 'daily_min',
                      'Mike_Hydro_RFE', 'Mike_Hydro_CHIRP', 'Mike_Hydro_IMERG'];

      models.forEach(model => {
        if (record[model] !== undefined && record[model] !== null && record[model] !== '' && record[model] !== '0') {
          const value = parseFloat(record[model]);
          if (!isNaN(value) && value > 0) {
            parsedRecord[model] = value;
          }
        }
      });
  ...
  const availableModels = React.useMemo(() => {
    const models = {
      'Floodproof': { key: 'Floodproof', color: '#9C27B0', name: 'Floodproof' },
      'GeoSFM': { key: 'GeoSFM', color: '#2196F3', name: 'GeoSFM' },
      'daily_avg': { key: 'daily_avg', color: '#4CAF50', name: 'Daily Avg' },
      'daily_max': { key: 'daily_max', color: '#FF9800', name: 'Daily Max' },
      'daily_min': { key: 'daily_min', color: '#00BCD4', name: 'Daily Min' },
      'Mike_Hydro_RFE': { key: 'Mike_Hydro_RFE', color: '#E91E63', name: 'Mike Hydro RFE' },
      'Mike_Hydro_CHIRP': { key: 'Mike_Hydro_CHIRP', color: '#FF5722', name: 'Mike Hydro CHIRP' },
      'Mike_Hydro_IMERG': { key: 'Mike_Hydro_IMERG', color: '#795548', name: 'Mike Hydro IMERG' }
    };

    const available = [];
    Object.entries(models).forEach(([key, config]) => {
      const hasData = chartData.some(item => item[key] !== undefined && item[key] !== null);
      if (hasData) {
  ...
  if (availableModels.length === 0) {
    return <div className="chart-no-data" style={{ padding: '20px', textAlign: 'center' }}>No valid forecast data to display.</div>;
  }

  // Calculate Y-axis domain
  const allValues = chartData.flatMap(item =>
    availableModels.map(model => item[model.key]).filter(v => v != null && !isNaN(v))
  );
  ...
  const todayDataPoint = React.useMemo(() => {
    ...
    for (let i = 0; i < chartData.length; i++) {
      const dataPoint = chartData[i];
      const diff = Math.abs(dataPoint.timestamp.getTime() - currentDate.getTime());
    ...
    <div className="chart-container" ref={chartRef}>
      <ResponsiveContainer width="100%" height={height}>
        <LineChart data={chartData} margin={{ top: 5, right: 5, left: 45, bottom: 30 }}>
          ...
          {/* Render a line for each available model */}
