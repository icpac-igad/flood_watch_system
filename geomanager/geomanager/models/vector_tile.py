...
class PopupFieldBlock(blocks.StructBlock):
    ...
    hidden = blocks.BooleanBlock(required=False, default=False, label=_("Hidden (for chart data)"))
...
class VectorTileLayer(BaseTileLayer):
    ...
    def interaction_config(self):
        ...
        for popup_field in self.popup_config:
            ...
            # Include hidden flag for chart data fields
            if popup_field.value.get("hidden"):
                field_config["hidden"] = True
            config["output"].append(field_config)
