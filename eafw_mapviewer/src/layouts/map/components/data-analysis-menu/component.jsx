...
class DataAnalysisMenu extends PureComponent {
  ...
  getLinks = () => {
    const { links, clearAnalysisError, setMainMapSettings, setMapSettings, hidden, showAnalysis } = this.props;
    ...
    return links.map((l) => ({
      ...
      onClick: () => {
        setMainMapSettings({
          ...
          hideLegend: (showAnalysis && l.active && !hidden) || (!showAnalysis && l.active && !hidden),
        });
