...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("home", "0032_update_clip_to_admin_function"),
    ]

    operations = [
        migrations.RunSQL(
            sql=r"""
CREATE OR REPLACE FUNCTION gha.multimodal_points_clustered(z integer, x integer, y integer, cluster_zoom integer DEFAULT 7, date text DEFAULT NULL::text)
 RETURNS bytea
 LANGUAGE plpgsql
...
DECLARE
    ...
    mvt bytea;
BEGIN
    -- Get the date to query - either specified or latest available
    IF date IS NOT NULL AND date != '' THEN
        BEGIN
            query_date := date::date;
        EXCEPTION WHEN OTHERS THEN
            -- Invalid date string (e.g. '{{time}}' placeholder) - fall back to latest
    ...
    -- Get first future forecast date for map display (alert levels)
    ...
    IF z >= cluster_zoom THEN
        -- Individual points view at high zoom levels
        ...
        FROM (
            SELECT
                ...
                f.mike_hydro_chirp,
                f.mike_hydro_imerg,
                -- Include ALL forecast dates (historical + future) for the chart
                COALESCE((
    ...
    ELSE
        -- Clustered view at low zoom levels
        SELECT ST_AsMVT(tile, 'gha.multimodal_points_clustered', 4096, 'mvt_geom') INTO mvt
        FROM (
            SELECT
                min(point_id) as id,
...
CREATE OR REPLACE FUNCTION gha.multimodal_points_clustered_whca(z integer, x integer, y integer, cluster_zoom integer DEFAULT 7, date text DEFAULT NULL::text, country_name text DEFAULT NULL::text, region_name text DEFAULT NULL::text, district_name text DEFAULT NULL::text)
 RETURNS bytea
 LANGUAGE plpgsql
 STABLE PARALLEL SAFE
AS $function$
    ...
    IF z >= cluster_zoom THEN
        ...
        FROM (
            SELECT
                ...
                f.mike_hydro_chirp,
                f.mike_hydro_imerg,
                -- Include ALL forecast dates for the chart
                COALESCE((
                    SELECT json_agg(json_build_object(
                        'date', fc.forecast_date,
                        'daily_avg', fc.daily_avg,
...
CREATE OR REPLACE FUNCTION gha.multimodal_points_by_project(z integer, x integer, y integer, cluster_zoom integer DEFAULT 7, date text DEFAULT NULL::text, project_countries text DEFAULT NULL::text, country_name text DEFAULT NULL::text, region_name text DEFAULT NULL::text, district_name text DEFAULT NULL::text)
 RETURNS bytea
 LANGUAGE plpgsql
 STABLE PARALLEL SAFE
AS $function$
...
BEGIN
    ...
    -- Admin drill-down: override filter_geom with more specific boundary
    IF district_name IS NOT NULL AND district_name != '' THEN
...
CREATE OR REPLACE FUNCTION gha.multimodal_points_by_basin(z integer, x integer, y integer, cluster_zoom integer DEFAULT 7, date text DEFAULT NULL::text, basin_id bigint DEFAULT NULL::bigint)
 RETURNS bytea
 LANGUAGE plpgsql
 STABLE PARALLEL SAFE
AS $function$
        ...
        FROM (
            SELECT
                ...
                COALESCE((
                    SELECT json_agg(json_build_object(
                        'date', fc.forecast_date,
                        'daily_avg', fc.daily_avg, 'daily_max', fc.daily_max, 'daily_min', fc.daily_min,
                        'GeoSFM', fc.geosfm, 'Floodproof', fc.floodproof,
                        'Mike_Hydro_RFE', fc.mike_hydro_rfe, 'Mike_Hydro_CHIRP', fc.mike_hydro_chirp,
                        'Mike_Hydro_IMERG', fc.mike_hydro_imerg
                    ) ORDER BY fc.forecast_date)
                    FROM gha.multimodal_forecasts fc
                    WHERE fc.point_id = cp.point_id AND fc.data_date = query_date
                ), '[]'::json)::text as forecasts_json,
                CASE WHEN f.daily_avg >= 450 THEN 'emergency' WHEN f.daily_avg >= 300 THEN 'alarm' WHEN f.daily_avg >= 150 THEN 'warning' ELSE 'normal' END as alert_level,
                CASE WHEN f.daily_avg >= 450 THEN 4 WHEN f.daily_avg >= 300 THEN 3 WHEN f.daily_avg >= 150 THEN 2 ELSE 1 END as alert_priority,
                1 as point_count,
                ST_AsMVTGeom(ST_Transform(cp.geom, 3857), tile_bbox_3857, 4096, 64, true) AS mvt_geom
            FROM gha.multimodal_control_points cp
            LEFT JOIN gha.multimodal_forecasts f
...
CREATE OR REPLACE FUNCTION gha.multimodal_points_by_admin(z integer, x integer, y integer, cluster_zoom integer DEFAULT 10, country_name text DEFAULT NULL::text, region_name text DEFAULT NULL::text, district_name text DEFAULT NULL::text)
 RETURNS bytea
 LANGUAGE plpgsql
 STABLE PARALLEL SAFE
AS $function$
...
BEGIN
    ...
    -- Build filter geometry based on admin hierarchy
    IF district_name IS NOT NULL AND district_name != '' THEN
        SELECT geom INTO filter_geom FROM gha.admin2
