...
class Dataset(TimeStampedModel, AdminSortable):
    ...
    time_parameter = models.CharField(
        ...
        blank=False,
        null=False,
        verbose_name=_("Parameter name for WMS"),
        help_text=_("The query parameter name to use for date/time in WMS requests (e.g. 'time', 'date')"),
    )

    time_prefix = models.CharField(
        max_length=50,
        blank=True,
        null=True,
        verbose_name=_("Prefix for time param value"),
        help_text=_("Optional prefix to prepend to the time value in WMS requests. Leave blank for no prefix."),
    )

    watcher_config = models.ForeignKey(
        "WatcherConfig",
        ...
        on_delete=models.SET_NULL,
        related_name="datasets",
        help_text=_("Select a watcher configuration to enable automated updates for this dataset"),
    )
    ...
    last_watcher_update = models.DateTimeField(
        null=True,
        blank=True,
        editable=False,
        verbose_name=_("Last Watcher Update Time"),
        help_text=_("Timestamp of when the watcher last checked this dataset (internal use, not editable)"),
    )
    ...
    panels = [
        ...
        FieldPanel("time_parameter"),
        FieldPanel("time_prefix"),
        FieldPanel("watcher_config"),
    ...
    def get_parser_config(self):
        """Get parser config from watcher_config if available."""
        if self.watcher_config:
            return self.watcher_config.get_parser_config()
        return {}
    ...
    def get_wms_datasets_index_json(self, request=None):
        date_format = JS_TO_PYTHONIC_DATE_FORMAT_CHOICES[self.wms_layers.first().date_format]
        date_format = date_format if date_format is not None else "%Y-%m-%d"

        # Get all timestamps (already in UTC from database)
        timestamps = sorted(
            self.wms_datasets.all().values_list("datetime", flat=True),
            reverse=True,
        )
        ...
        return {
            ...
            "minzoom": 0,
            "maxzoom": 20,
            "time_parameter": self.time_parameter,
            "timestamps": [
                timestamp.strftime(date_format) for timestamp in timestamps
            ],  # Use configured date_format, operates on UTC datetime directly
        }
