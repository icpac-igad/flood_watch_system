...
class BaseTileLayer(TimeStampedModel, ClusterableModel, BaseLayer):
    ...
    def tile_url(self):
        ...
        if query_str:
            # Check if base_url already has query params (contains '?')
            separator = "&" if "?" in tile_url else "?"
            tile_url = f"{tile_url}{separator}{query_str}"
    ...
    def get_legend_config(self, request):
        ...
        if legend_block:
            ...
            for item in data.get("items"):
                # Support both "value" (standard block) and "name" (custom format) fields
                legend_item = {
                    "name": item.get("value") or item.get("name"),
                    "color": item.get("color"),
                }
                # Support custom icon URL (not from sprite)
                if item.get("icon"):
                    legend_item["icon"] = item.get("icon")
                config["items"].append(legend_item)
