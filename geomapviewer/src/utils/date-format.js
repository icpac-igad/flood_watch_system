...
/**
 * Converts Python strftime format to date-fns format.
 * Python strftime uses % prefixed codes, while date-fns uses letter patterns.
 *
 * @param {string} strftimeFormat - Python strftime format string (e.g., "%Y-%m-%d")
 * @returns {string} date-fns compatible format string (e.g., "yyyy-MM-dd")
 */
export function strftimeToDateFns(strftimeFormat) {
  if (!strftimeFormat) return strftimeFormat;
...
export function dFormatter(date, format, asPeriod) {
  ...
  if (format && format.includes("%")) {
    dateFnsFormat = strftimeToDateFns(format);
  }

  let formated = dateFormat(dateObj, dateFnsFormat);
