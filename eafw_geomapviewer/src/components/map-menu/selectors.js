...
// Local getEmbed selector to avoid circular dependency with layout selectors
...
export const getActiveSectionWithData = createSelector(
  ...
  (section) => {
    ...
    // Removed: filtering out subcategories without datasets caused active subcategories
    // with no data yet to disappear once any sibling subcategory received data.
    // Subcategories are already filtered by `active` in config-provider/actions.js,
    // and the component renders a "No datasets available" fallback for empty ones.
...
// Helper to filter out boundary-layers category
export const getNonBoundaryCategories = createSelector(
  [getDatasetCategories],
  (categories) =>
    categories && categories.filter((c) => c.category !== 'Boundary Layers')
);
